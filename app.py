...
from OpenSSL.crypto import FILETYPE_PEM #S21 - pip install pyOpenSSL
...
from werkzeug.utils import secure_filename #imagen
UPLOAD_FOLDER = 'static\\uploads' #imagen
ALLOWED_EXTENSIONS = {'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif'} #imagen
from base64 import b64encode #imagen

app = Flask( __name__ )
app.secret_key = os.urandom( 24 )

app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER #imagen
...
def allowed_file(filename):  #imagen
    return '.' in filename and \
           filename.rsplit('.', 1)[1].lower() in ALLOWED_EXTENSIONS

def convertToBinaryData(filename):  #imagen
    #Convierte datos digitales a archivos
    with open(filename, 'rb') as file:
        blobData = file.read()
    return blobData

#def writeTofile(data, filename):  #imagen
#    # Convert binary data to proper format and write it on Hard Disk
#    with open(filename, 'wb') as file:
#        file.write(data)
#    print("Archivo blob guardado en: ", filename, "\n")

@app.route('/uploadfile', methods=['GET', 'POST']) #imagen
def uploadfile():
    if request.method == 'POST':
        # Probar si el POST tiene un archivo
        if 'file' not in request.files:
            flash('No se recibió ningúna petición de archivo')
            return redirect(request.url)
        file = request.files['file']
        # Si el usuario no selecciona un archivo, 
        # el explorador envía una solicitud de archivo
        # sin nombre
        ...
        if file and allowed_file(file.filename):
            #Guardado de archivos en carpeta de la aplicación
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))

            #Guardado de archivos en base de datos
            db = get_db()
            db.execute(
                'INSERT INTO imagenes (archivo, ubicacion)'
                ' VALUES (?, ?)',
                (convertToBinaryData(os.path.join(app.config['UPLOAD_FOLDER'], filename)),os.path.join(app.config['UPLOAD_FOLDER'], filename))
            )
            db.commit()
            flash( "Imagen guardada en la base de datos" )
...
@app.route('/showfiles') #imagen
def showfiles():
    db = get_db()
    proc_images = []
    images = db.execute("SELECT id, archivo, ubicacion FROM imagenes").fetchall()
    for i in images:
        #i[0]: id, i[1]:imagen blob, i[2]:ruta imagen
        proc_images.append([i[0],b64encode(i[1]).decode("utf-8"),i[2]])
    return render_template('showfiles.html',imagenes=proc_images)

if __name__ == '__main__':
    app.run( host='127.0.0.1', port =443, ssl_context=('micertificado.pem', 'llaveprivada.pem'),debug=True  ) #S21
